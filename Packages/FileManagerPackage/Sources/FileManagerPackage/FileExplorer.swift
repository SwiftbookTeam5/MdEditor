...
public protocol IFileExplorer {
	...
	/// - Returns: Файл или ошибка.
	func createNewFile(at url: URL, fileName: String, fileBody: String) -> Result<File, Error>
}

public final class FileExplorer: IFileExplorer {
	...
	/// - Parameter url: Путь;
	/// - Returns: массив файлов содержащихся по пути URL или ошибка.
	public func contentsOfFolder(at url: URL) -> Result<[File], Error> {
		var files = [File]()
	...
	///   - name: имя папки.
	/// - Returns: Файл или ошибка.
	public func createFolder(at url: URL, withName name: String) -> Result<File, Error> {
		do {
	...
	///   - url: Путь, где создаем новый файл;
	///   - fileName: Имя нового файла;
	///   - fileBody: Тело нового файла.
	/// - Returns: Файл или ошибка.
	public func createNewFile(at url: URL, fileName: String, fileBody: String) -> Result<File, Error> {
		let newFileUrl = url.appendingPathComponent("\(fileName)")
		do {
			try fileBody.write(to: newFileUrl, atomically: true, encoding: fileEncoding)
